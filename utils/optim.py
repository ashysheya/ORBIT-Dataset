...
def nll_loss(test_logits, test_labels, reduction='mean'):
    return F.nll_loss(test_logits, test_labels, reduction=reduction)
...
def init_optimizer(model, lr, optimizer_type='adam', extractor_scale_factor=1.0):
    ...
    # optimizer = optimizer_fn([
    #                     {'params': list(base_params)[:-2]},
    #                     {'params': list(base_params)[-2:], 'lr': lr*5}
    #                     ], lr=lr)
    optimizer.zero_grad()
    return optimizer
